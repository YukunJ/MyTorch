...
class RNNCell(object):
    ...
    def forward(self, x, h):
        ...
        """
        h_prime = self.activation(x @ self.W_ih.T + self.b_ih + h @ self.W_hh.T + self.b_hh)
        return h_prime
    ...
    def backward(self, delta, h, h_prev_l, h_prev_t):
        ...
        # recal the forward formulat:
        # h_prime = tanh(z)
        # z = x @ self.W_ih.T + self.b_ih + h @ self.W_hh.T + self.b_hh
        # the x is called h_prev_l here
        # the h is called h_prev_t here
        # here we already have dz
